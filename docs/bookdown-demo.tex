...
\[= \sum_{i=1}^{n} [y_i \frac{1}{p(\beta ^T x_i)} p(\beta ^T x_i)(1-p(\beta ^T x_i))x_i+(1-y_i) \frac{1}{1-p(\beta ^T x_i)}(-1)p(\beta ^T x_i)(1-p(\beta ^T x_i))x_i] \]
\[= \sum_{i=1}^{n} [y_i \frac{1}{p(\beta ^T x_i)} p(\beta ^T x_i)(1-p(\beta ^T x_i))x_i-(1-y_i) \frac{1}{1-p(\beta ^T x_i)}p(\beta ^T x_i)(1-p(\beta ^T x_i))x_i] \]
\[= \sum_{i=1}^{n} [y_i (1-p(\beta ^T x_i))x_i-(1-y_i) p(\beta ^T x_i)x_i]=\sum_{i=1}^{n} [y_i-y_ip(\beta ^T x_i)-p(\beta ^T x_i)+y_i p(\beta ^T x_i)]x_i\]
\[=\sum_{i=1}^{n} [y_i-p(\beta ^T x_i)]x_i=\sum_{i=1}^{n} [y_i-\frac{e^{\beta^Tx_i}}{1+e^{\beta^Tx_i}}]x_i\]

I will use probit in the following data simulation (3 IVs and 1 DV) and
Newton Raphson analysis. I simulated 500 observations.
...
\begin{verbatim}
## Warning in while (difference > tolerance & iteration < max_its) {: the condition
