...
calculate the expectation value of \(h(x)\), and \(x \sim f(x)\).
...
of \(e{-(x-2)}\), and then insert them into the density function of
...
\tightlist
\item
  Solve the \(x^3-5=0\) Note that, this is obviously not a maximization
  problem. In contrast, it involves a function with zero. As we can see,
  we can think it as the first order of Taylor approximation. That is,
  itself is a \(f^{'}(x)=x^3-5=0\). As we can see the following plot, it
  converts pretty quick.
\end{enumerate}
...
\includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-20-3.pdf}
