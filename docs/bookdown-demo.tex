...
Remarks (from left to right): (1) Parameter is
\(\theta = \beta ^T x_i\)\$ (Not \(\mu\)) (2)
\(\mu=p(y=1)=\frac{e^{\beta^Tx_i}}{1+e^{\beta^Tx_i}}\) (NOT logit!)
(3)link function (i.e., \(g(\mu)\)) = logit = logarithm of odds = log
\(\frac{Event - Happened }{Event - Not - Happened}\) . (4)
