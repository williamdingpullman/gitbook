...
For \(n\) time Bernoulli (i.e., Binomial), its canonical link function
is logit.

\[log \frac{p(y_i=1)}{1-p(y_i=1)}=\beta^Tx_i\] Where,

\[p(y_i=1)= \frac{e^{\beta^Tx_i}}{1+e^{\beta^Tx_i}}\]

We can rewrite it as follows.

\[\beta^Tx_i=log\frac{\mu}{1-\mu}=g(\mu)\] (Side note: logit = log odds
= log \(\frac{Event - Happened }{Event - Not - Happened}\))

Where,

\[\mu=p(y=1)=\frac{e^{\beta^Tx_i}}{1+e^{\beta^Tx_i}}\] Thus, we know
that \(\mu\) or \(p(y=1)\) is the mean function. Recall that, \(n\)
trails of coin flips, and get \(p\) heads. Thus \(\mu = \frac{p}{n}\).
