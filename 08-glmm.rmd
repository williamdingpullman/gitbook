...
The idea is that, we can generate $x_i$ from exponential distribution of $e{-(x-2)}$, and then insert them into the density function of $\frac{\frac{1}{2 \pi} e^{-\frac{1}{2}x^2}}{e^{-(x-2)}}$. Thus, as you can see, importance sampling is based on expetation value (law of large numbers:  If the same experiment or study is repeated independently a large number of times, the average of the results of the trials must be close to the expected value). We can use it to calculate integral based on link of the definition of expected value. 
