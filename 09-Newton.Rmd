...
There are different ways to understand Newton Raphson method, but I found the method fo geometric the most easy way to explain. I use the following figure from a online PDF from UBC (see the reference for the source.) 

![THe illustration figure](Newton.jpg) 


Specifically, suppose that you want to calculate the root of a function $f(x)=0$. We assume the root is $r$. However, we do not that, and we randomly guess a point of $a$. Thus, we can get a tangent line with slope of $f^{'}(a)$ and a point of $(a,f(a))$. Since we know the slope and one of its point, we can write the function for this tangent line. 

$$y-f(a)=f^{'}(a)(x-a)$$
To calculate the $x-intercept$, namely the $b$, we can set $y=0$, and get the following:

$$-f(a)=f^{'}(a)(x-a) \Rightarrow x (or, b)= a-\frac{f(a)}{f^{'}(a)}$$
If there significant difference of $|a-b|$, we know that our orginal guess of $a$ is not good. We better use $b$ as the next guess, and calculate its tangent line again. To generate, we can write it as follows.
...
Often, $x$ is not just a single unknow parameter, but a vector. We can write it as follows.
