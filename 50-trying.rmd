...
for (k in 1:n)
 {
  ...
  for(j in 1:n)
      { # Make sure the k is not equal j, otherwise 0
...
Using the Newton Raphson, the following code calculates the basic logistic model, without any random effect. As we can see, it gets the same result as the R function of GLM. Note that, the function of Expit and the variables of y and are from the last block of R code.
